...
CMD="${KAMBIN} -P ${KAMPID} -w ${KAMRUN} -Y ${KAMRUN} -f ./kamailio-tgroup0001.cfg -a no -ddd -E ${KAMEXTRA}"
echo "${CMD}"
eval "${CMD}" 2>&1 | tee /tmp/kamailio-tgroup0001.log &
ret=$?
sleep 1
...
CMD="${KAMBIN} -P ${KAMPID} -w ${KAMRUN} -Y ${KAMRUN} -f ./kamailio-tgroup0001.cfg -A WITH_MULTIDOMAIN -a no -ddd -E ${KAMEXTRA}"
echo "${CMD}"
eval "${CMD}" 2>&1 | tee /tmp/kamailio-tgroup0001.log &
ret=$?
sleep 1
